...
# Set $JANUSGRAPH_CFG to $JANUSGRAPH_HOME/conf if unset
if [[ -z "$JANUSGRAPH_CFG" ]]; then
  cd -P ${JANUSGRAPH_HOME}/conf
  JANUSGRAPH_CFG=$(pwd)
fi
...
JANUSGRAPH_SERVER_CMD=org.janusgraph.core.server.MainKt
...
if [[ -z "$JANUSGRAPH_YAML" ]]; then
  JANUSGRAPH_YAML=$JANUSGRAPH_CFG/janusgraph-server.yaml
fi
...
case "$1" in
  ...
  stop)
    ...
    ;;
  help|usage)
    echo "Usage: $0 {start|stop|restart|status <group> <artifact> <version>|<conf file>}"
    echo
